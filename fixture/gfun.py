...
from gbcall import VirtualFile, defineExpose,types
...
@defineExpose(
    input=[
        ...
        types.Text],
    output=[types.Text,types.File,types.File,types.File]
)
def gFuncCalc(
        S1, n,frac_num , frac_spac , Hw , Hp , Q , tp , E , v , ISIP , P_i , t_i , P_j , t_j , m , frac_type):
    ...
    sys.stdout =log
    # name a= input("算例文件夹备注：")  # 手动输入
    # file = ".\\result\\" + get_time_str() + "-" + name
    ...
    #     ]
    # }
    # js_list = json.loads(json.dumps(jsonstr))['data']

    # js_list从json文件读取，或者直接作为request的参数传入
    # data_json=[]
    ...
    img3d=VirtualFile(".png")
    plt.savefig(img3d.name)
    # if choose == "y":
    #     plt.show()
    ...
    img2d=VirtualFile(".png")
    plt.savefig(img2d.name)
    # if choose == "y":
    #     plt.show()
    ...
    paragraph.add_run().add_picture(
        ...
        img3d.name, width=Cm(12), height=Cm(8)
    )
    paragraph.add_run().add_picture(
        # file + "\\2D_" + time_str + ".png", width=Cm(12), height=Cm(8)
        img2d.name, width = Cm(12), height = Cm(8)
    )
    ...
    doc.add_section()
    docFile=VirtualFile()
    doc.save(docFile.name)
    console.print("分析报告已生成！", style="red")
    sys.stdout=tmpStdout
    return log.getvalue(),img3d,img2d,docFile




    # 10,2,2,20,20,20,0.2,20,30000,0.25,12.5958,12.5958,20,10.052,12509.7,1,"PKN"
